...
ZBOSS_DECLARE_DEVICE_CTX_1_EP(temperature_sensor_ctx, temperature_sensor_ep);
 ...
 K_THREAD_DEFINE(com_task_id, STACKSIZE, com_task, NULL, NULL, NULL, -1 , 0, 1000); 
...
zb_time_t     timeout_bi;
  	if(!ZB_JOINED())
	{
	  	if ((ZB_SCHEDULE_GET_ALARM_TIME(Recon_timer_cb, ZB_ALARM_ANY_PARAM, &timeout_bi) != RET_OK)||(timeout_bi == 0)) 
  		{
		ZB_SCHEDULE_APP_ALARM(Recon_timer_cb, 0, ZB_MILLISECONDS_TO_BEACON_INTERVAL(DELAY_TOTAL_ZI_RECONNECT_MS));
  		}	
	}	    
  	else
  	{
	ZB_SCHEDULE_APP_ALARM_CANCEL(Recon_timer_cb, ZB_ALARM_ALL_CB); 	  
	LOG_INF("zi_reboot===*  STOOPPPPPP   *==Zigbee");
  	}
}
...
zb_int16_t new_temp_value=TEMPERATURE_NO_MEASURE;
	while(1)
	{	
	k_sleep(K_MSEC(1000));
	/* Get new temperature measured value */
		if(new_temp_value != ADC_Value_T[0])
		{
		new_temp_value = ADC_Value_T[0];	
			if ((new_temp_value & TEMPERATURE_ERROR_MASK)!=TEMPERATURE_ERROR_LABEL)
			{
			new_temp_value*=100;
			}
...
void main(void)
{
	...
	disable_pin_reset_and_block();

	emfat_init(&emfat, "Zep", entries);
    user_mem_init ();
    mh_Buf_Init ();
	mh_usb_init();
